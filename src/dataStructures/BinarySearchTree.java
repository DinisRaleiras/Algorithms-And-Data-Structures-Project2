...
{                                                                   
    ...
    **
    ...
    void linkSubtreeInsert(BSTNode<Entry<K,V>> node, BSTNode<Entry<K,V>> parent) {
        if ( parent == null )
            ...
            if (node != null) {
                ...
                if (node.getElement().getKey().compareTo(parent.getElement().getKey()) <= 0)
                    parent.setLeft(node);
                else
    ...
    /**
     * Writes the state of the object, we rewrite this method to avoid writing the tree in a recursive way
     * @param s
    ...
    /**
     * Reads the state of the object, we rewrite this method to avoid reading the tree in a recursive way
     * @param s
     * @throws java.io.IOException
